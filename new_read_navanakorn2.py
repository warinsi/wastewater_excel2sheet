...
def open_file():
    ...
    if filename:
        try:
            ...
            month_file_name = filename.split("Results_SKC_Navanakorn_")
            month_name = month_file_name[1].split(".xlsx")
            name_sheet = "Results (Wastewater)_" + month_name[0]
            df = pd.read_excel(filename, name_sheet)
            print("df: ",df)
